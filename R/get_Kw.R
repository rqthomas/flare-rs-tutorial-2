...
get_kw_US <- function(bbox, validate = TRUE, lake_row = NA){
  ...
  lakeinfo <- read_csv("supporting_data/lake_information.csv")
  mylake <- lakeinfo |>
  ...
  lagos_qual <- read_csv("supporting_data/LAGOS_US_LANDSAT_Predictions_AVERAGED.csv", show_col_types = F)
...
get_kw_global <- function(bbox, validate = TRUE){
  ...
  if(dim(mylake)[1] != 0){
    ...
    message("Your lake may not exist in the database.")
  }
}
